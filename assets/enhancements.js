(function () {
  ...
  function addRunButton(panel, block, lang, version) {
    ...
    btn.textContent = lang === 'python' ? 'Esegui Python' : 'Esegui Go';
  ...
  var CHAPTERS = [
    { n: '01', title: "HTTP e CORS", time: '3-4 ore', keys: 'http cors headers methods status tls https cookies preflight request response' },
    { n: '02', title: "Routing nel backend", time: '2-3 ore', keys: 'routing path parameters query strings mux dynamic routes' },
    { n: '03', title: "Serializzazione e deserializzazione", time: '2-3 ore', keys: 'serialization deserialization json protobuf encoding decoding marshal' },
    { n: '04', title: "Autenticazione e autorizzazione", time: '3-4 ore', keys: 'authentication authorization jwt oauth tokens session cookies rbac' },
    { n: '05', title: "Convalide e trasformazioni", time: '2-3 ore', keys: 'validation transformation sanitization input schema zod pydantic' },
    { n: '06', title: "Controller, servizi e middleware", time: '2-3 ore', keys: 'controllers services repositories middleware request context layers' },
    { n: '07', title: "Progettazione API (REST)", time: '3-4 ore', keys: 'api rest restful design versioning pagination hateoas idempotency' },
    { n: '08', title: "Database", time: '2-3 ore', keys: 'database postgres sql orm queries transactions connection pooling indexes' },
    { n: '09', title: "Memorizzazione nella cache", time: '2-3 ore', keys: 'caching redis memcached cache invalidation ttl lru write-through' },
    { n: '10', title: "Code di attività e lavori in background", time: '3-4 ore', keys: 'task queues background jobs workers async redis sidekiq celery' },
    { n: '11', title: "Ricerca full-text (Elasticsearch)", time: '2-3 ore', keys: 'elasticsearch search full-text inverted index fuzzy tokenizer analyzer' },
    { n: '12', title: "Gestione degli errori e tolleranza agli errori", time: '2-3 ore', keys: 'error handling retry circuit breaker fault tolerance resilience fallback' },
    { n: '13', title: "gRPC e comunicazione tra servizi", time: '3-4 ore', keys: 'grpc protobuf microservices inter-service rpc streaming unary' },
    { n: '14', title: "Gestione della configurazione", time: '2-3 ore', keys: 'configuration environment variables yaml dotenv secrets config management' },
    { n: '15', title: "Registrazione e osservabilità", time: '2-3 ore', keys: 'logging monitoring observability structured logs metrics tracing' },
    { n: '16', title: "Spegnimento grazioso", time: '2-3 ore', keys: 'graceful shutdown signals sigterm sigint sigkill process lifecycle' },
    { n: '17', title: "Sicurezza del back-end", time: '2-3 ore', keys: 'security xss csrf sql injection cors csp hsts helmet' },
    { n: '18', title: "Scalabilità e prestazioni (Parte 1)", time: '2-3 ore', keys: 'scaling performance horizontal vertical load balancing sharding' },
    { n: '19', title: "Scalabilità e prestazioni (Parte 2)", time: '2-3 ore', keys: 'scaling performance cdn rate limiting connection pooling' },
    { n: '20', title: "Concorrenza e parallelismo", time: '2-3 ore', keys: 'concurrency parallelism goroutines asyncio threads io-bound cpu-bound' },
    { n: '21', title: "Docker, K8 e CI/CD", time: '2-3 ore', keys: 'docker kubernetes k8s cicd containers deployment dockerfile pods' },
    { n: '22', title: "Test automatizzati", time: '2-3 ore', keys: 'testing unit integration e2e test coverage mocking tdd' },
    { n: '23', title: "Broker di messaggi e Kafka", time: '2-3 ore', keys: 'kafka message brokers event streaming pub-sub consumers producers topics' },
    { n: '24', title: "WebSocket e tempo reale", time: '2-3 ore', keys: 'websockets real-time ws upgrade handshake bidirectional persistent' }
  ];
  ...
  function initSearch() {
    ...
    function renderResults(query) {
      ...
      if (matches.length === 0) {
        resultsEl.innerHTML = '<div class="search-no-results">Nessun capitolo trovato.</div>';
        return;
      }
  ...
  var THEME_LABELS = { default: 'Originale', neutral: 'Originale', paper: 'Originale', dark: 'Scuro', light: 'Chiaro' };
  ...
  function applyTheme(theme, animate) {
    ...
    if (btn) {
      btn.innerHTML = THEME_LABELS[theme] || 'Originale';
    }
  }
  ...
  function initFloatingDock() {
    ...
    dock.setAttribute('aria-label', 'Strumenti di studio e navigazione rapida');
    ...
    var html = '<button class="dock-btn dock-theme-btn" id="dockThemeBtn" aria-label="Switch color theme (Original / Dark / Light)" title="Switch Theme (Original / Dark / Light)">' +
      (THEME_LABELS[currentTheme] || 'Originale') +
      '</button>' +
      ...
      '</button>' +
      '<button class="dock-btn dock-top-btn" id="dockTopBtn" title="Torna in alto" aria-label="Torna in cima alla pagina">' +
  ...
  function initHighlighter() {
    ...
    bubble.innerHTML = '<button class="bubble-dot-btn dot-rust" data-color="rust" title="Highlight Blue (H)"></button>' +
      '<button class="bubble-dot-btn dot-amber" data-color="amber" title="Evidenzia in ambra"></button>' +
      '<button class="bubble-dot-btn dot-emerald" data-color="emerald" title="Evidenzia in verde"></button>' +
      '<button class="bubble-btn" id="bubbleNoteBtn" title="Aggiungi nota (N)">Nota</button>' +
      '<button class="bubble-btn" id="bubbleCopyQuoteBtn" title="Copia selezione">Copia</button>';
    ...
    popover.innerHTML = '<div class="note-popover-header">' +
      ...
      '<button class="note-color-dot dot-rust active" data-color="rust" title="Blu"></button>' +
      '<button class="note-color-dot dot-amber" data-color="amber" title="Ambra"></button>' +
      '<button class="note-color-dot dot-emerald" data-color="emerald" title="Verde"></button>' +
      '</div>' +
      '<div style="display:flex;gap:6px;">' +
      '<button class="dock-btn" id="notePopoverCancelBtn" style="font-size:11px;padding:4px 10px;">Annulla</button>' +
      '<button class="dock-btn" id="notePopoverSaveBtn" style="font-size:11px;padding:4px 12px;background:var(--accent);color:#fff;border-color:var(--accent);">Salva nota</button>' +
      '</div>' +
      '</div>';
    ...
    tooltip.innerHTML = '<span class="tooltip-note" id="tooltipNotePreview"></span>' +
      '<button class="bubble-btn" id="tooltipEditBtn" title="Modifica nota">Modifica</button>' +
      '<button class="bubble-btn" id="tooltipRemoveBtn" title="Rimuovi evidenziazione" style="color:#ff6b57;">Rimuovi</button>' +
      '<button class="bubble-btn" id="tooltipViewNotesBtn" title="Visualizza nella barra laterale">Appunti</button>';
  ...
  window.masterclassCopyCode = function (btn) {
    ...
    function indicateCopied() {
      ...
      setTimeout(function () {
        ...
        if (copyText) copyText.textContent = 'Copia';
      }, 2000);
    }
  ...
  function renderObsidianMarkdown(rawText) {
    ...
    src = src.replace(/```([a-zA-Z0-9_\-\+\.]*)\n([\s\S]*?)```/g, function (match, lang, code) {
      ...
      var blockHtml = '<div class="md-codeblock-wrap">' +
        ...
        '  </div>' +
        '  <button type="button" class="md-code-copy-btn" onclick="window.masterclassCopyCode(this)" title="Copia frammento" aria-label="Copia il codice negli appunti">' +
        '    <span class="md-copy-icon">' + SVG_ICONS.copy + '</span> <span class="md-copy-text">Copia</span>' +
        '  </button>' +
        '</div>' +
  ...
  window.masterclassPreviewImage = function (src) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = '<div class="image-preview-backdrop"></div>' +
        ...
        '<img src="" id="masterclassPreviewImg" alt="Immagine ingrandita della nota" />' +
        '<button class="image-preview-close" title="Chiudi anteprima">&times;</button>' +
        '</div>';
  ...
  function attachPreviewTaskListeners() {
    ...
    preview.querySelectorAll('.md-task-item').forEach(function (taskEl, idx) {
      ...
      taskEl.title = 'Fai clic per cambiare lo stato della casella';
  ...
  function initNotesSidebar() {
    ...
    var activeLesson = getActiveLessonInfo();
    var activeLessonLabel = activeLesson.isChapter ? ('Active Lesson: ' + activeLesson.shortTitle) : 'Tutte le lezioni';
    var exportBtnLabel = activeLesson.isChapter ? ('Export Markdown (' + activeLesson.shortTitle + ')') : 'Esporta Markdown';
    ...
    drawer.innerHTML = '<div class="notes-sidebar-header">' +
      ...
      '<div class="notes-sidebar-title">Appunti di studio</div>' +
      '<span class="notes-save-status is-saved" id="notesSaveStatus" aria-live="polite">● Saved</span>' +
      '</div>' +
      '<div style="display:flex; align-items:center; gap:6px;">' +
      '<div class="notes-mode-toggle" role="tablist" aria-label="Modalità di visualizzazione degli appunti">' +
      '<button type="button" class="notes-mode-btn active" id="notesEditModeBtn" role="tab" aria-selected="true" aria-controls="lessonNotepadTextarea" aria-label="Modifica Markdown (Ctrl+Shift+E)" title="Modifica Markdown">' + SVG_ICONS.edit + ' <span>Modifica</span></button>' +
      '<button type="button" class="notes-mode-btn" id="notesPreviewModeBtn" role="tab" aria-selected="false" aria-controls="lessonNotepadPreview" aria-label="Anteprima (Ctrl+Shift+P)" title="Anteprima in stile Obsidian">' + SVG_ICONS.preview + ' <span>Anteprima</span></button>' +
      '</div>' +
      '<button class="dock-btn" id="sidebarCloseBtn" aria-label="Chiudi gli appunti" title="Chiudi gli appunti" style="font-size:13.5px; border:1px solid var(--line-strong); display:inline-flex; align-items:center; justify-content:center; padding:6px 14px; border-radius:8px;">Chiudi</button>' +
      '</div>' +
      '</div>' +
      ...
      '</div>' +
      '<div class="obsidian-toolbar" id="obsidianToolbar" role="toolbar" aria-label="Barra di formattazione Markdown">' +
      ...
      '<button type="button" class="obs-tool-btn" data-tool="task" aria-label="Insert Task list item" title="Task List (- [ ])">' + SVG_ICONS.task + ' <span>Attività</span></button>' +
      '<button type="button" class="obs-tool-btn" data-tool="bullet" aria-label="Insert Bullet list item" title="Bullet List (- item)">' + SVG_ICONS.list + ' <span>Elenco</span></button>' +
      '<div class="obs-tool-divider" role="separator" aria-orientation="vertical"></div>' +
      ...
      '<button type="button" class="obs-tool-btn obs-tool-btn-img" id="notepadImportImgBtn" aria-label="Importa immagine o incolla dagli appunti" title="Importa immagine (o incolla / trascina)">' + SVG_ICONS.image + ' <span>Immagine</span></button>' +
      '<input type="file" id="notepadImageFileInput" accept="image/*" style="display:none;" aria-hidden="true" />' +
      '<div style="flex:1;"></div>' +
      '<button type="button" class="obs-tool-btn obs-save-btn" id="notepadManualSaveBtn" aria-label="Salva subito gli appunti (Ctrl+S)" title="Salva gli appunti — Ctrl+S">' + SVG_ICONS.save + ' <span>Salva</span></button>' +
      '</div>' +
      '</div>' +
      '<!-- FULL HEIGHT TEXTAREA & PREVIEW -->' +
      '<div class="notes-notepad-body" id="notesNotepadBody">' +
      '<textarea class="notes-notepad-textarea" id="lessonNotepadTextarea" aria-label="Editor Markdown degli appunti" placeholder="Write your notes here in Obsidian Markdown...&#10;&#10;Tip: You can paste (Ctrl+V) or drag & drop images directly here!"></textarea>' +
      '<div class="notes-notepad-preview" id="lessonNotepadPreview" style="display:none;" aria-label="Anteprima degli appunti"></div>' +
      '</div>' +
      '<div class="notes-sidebar-footer">' +
      '<div class="notes-footer-stats" id="notesFooterStats" aria-live="polite">0 words · 0 chars</div>' +
      '<div style="display:flex; align-items:center; gap:8px;">' +
      '<button class="dock-btn reset-notes-btn" id="sidebarResetNotesBtn" aria-label="Cancella e reimposta gli appunti di questa lezione" title="Cancella gli appunti di questa lezione" style="font-size:13.5px; color:#ff6b57; border:1px solid rgba(255,107,87,0.35); background:transparent; display:inline-flex; align-items:center; justify-content:center; padding:8px 14px; border-radius:8px; cursor:pointer;">Svuota</button>' +
      '<button class="dock-btn export-md-btn" id="exportMdBtn" aria-label="Esporta gli appunti in un file Markdown" style="background:var(--accent); color:#fff; border-color:var(--accent); font-size:13.5px; padding:8px 16px; border-radius:8px;">' +
  ...
  function openNotesSidebar() {
    ...
    if (drawer && backdrop) {
      ...
      if (activeLessonText) {
        activeLessonText.textContent = activeLesson.isChapter ? ('Active Lesson: ' + activeLesson.shortTitle) : 'Tutte le lezioni';
      }
      var exportLabel = document.getElementById('exportMdBtnLabel');
      if (exportLabel) {
        exportLabel.textContent = activeLesson.isChapter ? ('Export Markdown (' + activeLesson.shortTitle + ')') : 'Esporta Markdown';
      }
  ...
  function showCodeBookmarkPopover(btn, card, targetId) {
    ...
    var html = '<div class="cb-popover-title">Salva frammento di codice</div>' +
      ...
      '      <small>' + (activeTab.isSaved ? 'Già salvato negli appunti (clic per salvare di nuovo)' : 'Save ' + escapeHtml(activeTab.label) + ' snippet to notes') + '</small>' +
      '    </div>' +
      '  </button>' +
      ...
      '      <small>' + (savedTabsCount === tabs.length ? 'Tutti i linguaggi salvati negli appunti' : 'Salva insieme le versioni in tutti i linguaggi') + '</small>' +
      '    </div>' +
      '  </button>';

    if (savedTabsCount > 0) {
      html += '  <button type="button" class="cb-popover-btn remove-btn" aria-label="Rimuovi dagli appunti i frammenti di questo blocco">' +
        '    <span class="cb-icon">' + SVG_ICONS.trash + '</span>' +
        '    <div class="cb-btn-body">' +
        '      <b>Rimuovi dagli appunti</b>' +
        '      <small>Scollega i frammenti salvati di questo blocco</small>' +
        '    </div>' +
        '  </button>';
    }
  ...
  function initCodeAndDiagramBookmarks() {
    ...
    codeBlocks.forEach(function (card, idx) {
      ...
      if (header && !header.querySelector('.code-bookmark-btn')) {
        ...
        btn.innerHTML = '+ Note';
        btn.title = 'Salva il codice negli appunti';
    ...
    diagrams.forEach(function (diag, idx) {
      ...
      if (diag.classList.contains('viz')) {
        ...
        if (cap && !diag.querySelector('.viz-bookmark-btn')) {
          ...
          btn.setAttribute('data-target-id', targetId);
          btn.innerHTML = '+ Save Diagram';
          btn.title = 'Salva il diagramma negli appunti';
          ...
          btn.addEventListener('click', function (e) {
            ...
            if (isAlreadySaved) {
              ...
              var title = diagVisual ? diagVisual.title : 'Diagramma architetturale';
              var excerpt = '';
        ...
        if (frame && !diag.querySelector('.viz-bookmark-btn')) {
          ...
          var title = figcap ? figcap.textContent.trim() : 'Diagramma architetturale';
          ...
          fbtn.title = 'Salva il diagramma negli appunti';
